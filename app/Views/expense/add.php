...
<div class="app">
  <div class="panel active" id="panel-add">
    <header class="app-header">
      <!-- Back button returns to the previous screen in browser history -->
      <button class="back-btn" onclick="history.back()">&#8592; Back</button>
      <h1>Add Expense</h1>
    </header>
    <!--Expense entry form: posts data to the save action-->
    <form action="/expense_tracker/public/expense/save" method="POST" id="add-form">
      ...
      <section class="add-section">
        ...
        <div class="cat-select-grid">
          <?php foreach ($categories as $c): ?>
            <!--Each category button sets the selected category in JS-->
        ...
        <!--Hidden field updated by JS when a category button is clicked-->
        <input type="hidden" name="category_id" id="selected_category">
      </section>
      <!--Amount input section using a custom keypad-->
      <section class="add-section">
        <h2>Amount (DKK)</h2>
        <div class="amount-display" id="amount-display">0</div>
        <!--Hidden amount input is submitted with the form-->
      ...
      <!--Expense details section for note and date-->
      <section class="add-section">
        <h2>Details</h2>
        ...
        <!--Submit button sends the completed expense to the server-->
        <button type="submit" class="btn-primary">Save Expense</button>
      </section>
    </form>
  </div>
  <!--Bottom navigation bar with Home, Add and Account links-->
...
<!--Javascript handles category select, keypad input, and form state -->
